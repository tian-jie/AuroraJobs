...
*加载后台任务列表
**/
function ScheduledTaskListController($scope, $http, $window) {
    var gridId = 'data_grid';
    $scope.LoadScheduledTaskList = function () {
        oUtils.jqGrid({
            gridId: gridId,
            url: '/ScheduledTask/List?r=' + Math.random(),
            dataIdKey: 'ScheduledTaskId',
            isPager: true,
            colNames: [
                        "操作",
                        "记录ID",
            ...
            colModel: [
                        { name: 'ScheduledTaskId', index: 'ScheduledTaskId', width: 120, formatter: $scope.formatModel, edittype: 'custom', hidden: false },
...
*后台任务新增视图控制器
**/
function ScheduledTaskAddController($scope, $http, $window) {
    $scope.model = {
        ScheduledTaskId: EmptyGUID,
        JobType: '',
        Name: '',
        AssemblyName: '',
        ClassName: '',
    ...
    $scope.SubmitAdd = function () {
        ...
        $http({
            method: 'POST',
            url: '/ScheduledTask/AddPost?r=' + Math.random(),
            data: $scope.model
        })
...
*后台任务修改视图控制器
**/
function ScheduledTaskUpdateController($scope, $http, $window) {

    var request = oUtils.urlParameter();
    var ScheduledTaskId = request["jobId"];
    ...
    $scope.LoadScheduledTaskInfo = function () {
        $scope.model.ScheduledTaskId = ScheduledTaskId;
        $http({
            method: 'GET',
            url: '/ScheduledTask/InfoData?r=' + Math.random(),
            cache: false,
            params: { ScheduledTaskId: ScheduledTaskId }
        })
       .success(function (result, status, headers, config) {
           if (result.success) {
    ...
    $scope.SubmitUpdate = function () {
        ...
        $http({
            method: 'POST',
            url: '/ScheduledTask/UpdatePost?r=' + Math.random(),
            data: $scope.model
        })
        .success(function (result, status, headers, config) {
            if (result.success) {
